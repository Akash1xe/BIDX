...
export default function NotificationCenter() {
  ...
  return <div className="notification-center"><div className="notification-center-toolbar"><p>Demo auction and payment activity is stored on this device. Backend delivery records appear here when available.</p>{unreadCount > 0 && <Button variant="outline" onClick={markAllRead}><CheckCheck /> Mark all read</Button>}</div><Tabs defaultValue="all"><TabsList><TabsTrigger value="all">All <span>{items.length}</span></TabsTrigger><TabsTrigger value="unread">Unread <span>{unreadCount}</span></TabsTrigger></TabsList><TabsContent value="all"><NotificationList items={items} readIds={readIds} markRead={markRead} empty="No notifications yet" /></TabsContent><TabsContent value="unread"><NotificationList items={unreadItems} readIds={readIds} markRead={markRead} empty="No unread notifications" /></TabsContent></Tabs></div>;
}
